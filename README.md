...
### Один роутер. Российские сайты — быстро. Зарубежные — открываются. Реклама — не показывается ни на одном устройстве в доме.
...
## 👋 Если ты пришёл сюда из России — давай по-честному

**Знакомо?**

- 📺 YouTube тормозит, видео не подгружается, реклама перед каждым роликом
- 📱 Включаешь VPN на телефоне — отваливаются Госуслуги, Сбер, Яндекс.Доставка, ЦБ
- 🏠 На каждом устройстве в доме нужно ставить и настраивать VPN отдельно
- 📺 На Smart TV вообще VPN не поставить — там «голый» интернет
- 💸 Платишь за 2–3 разных VPN-сервиса, потому что один не покрывает все устройства
- 🚫 Реклама везде — на сайтах, в приложениях, в YouTube

Это **не твоя проблема**. Это можно решить один раз — на уровне роутера, для всех устройств в доме сразу.
...
|---|---|
| YouTube тормозит, видео не грузится | Открывается на максимальной скорости |
| Госуслуги/Сбер/Озон отваливаются с VPN | Работают **всегда**, как с обычным интернетом |
| На каждом устройстве свой VPN, всё надо настраивать | Подключился к Wi-Fi — всё уже работает. На любом устройстве |
| Smart TV без VPN, дети смотрят с рекламой | Smart TV работает как все остальные устройства |
| Реклама в YouTube, на сайтах, в приложениях | Не показывается на любом устройстве в Wi-Fi |
| Платишь 3 раза за разные VPN-сервисы | Платишь один раз — за один сервер для всей семьи |

**В двух словах:** ты ставишь у себя дома специально настроенный роутер. Все устройства, которые подключаются к нему по Wi-Fi, автоматически получают свободный, безопасный интернет — без рекламы, без отвалов, без необходимости настраивать что-то на телефоне или планшете.
...
| # | Шаг | Время | Стоимость |
|---|---|---|---|
| 1 | Купить роутер **Cudy TR3000 v1** на [AliExpress](https://aliexpress.com/wholesale?SearchText=Cudy+TR3000) | 5 минут заказать + ~неделя ожидания | ~$45 разово |
| 2 | Прошить роутер OpenWrt — [инструкция](docs/10-upgrades.md) | ~30 минут с инструкцией | бесплатно |
| 3 | Купить подписку на VPN-сервер ([👇 кнопка ниже](#-сколько-это-стоит)) | ~5 минут | ~250₽/мес |
...
ssh root@192.168.1.1 'BS=/tmp/bootstrap.sh && \
  ...
  echo "fa812ddccbc6572bb2869522dcf7e2ace81d15d9836980c2f8100717a5a1a4cc  $BS" | sha256sum -c - && \
  sh "$BS"'
```
...
> **Подписка:** от ~250₽/мес — за VPN-сервер.
...
**Зато выгода считается легко.** В семье обычно 3–5 устройств с VPN: телефоны родителей, ноутбук, телевизор, планшет. По отдельности это 3–5 × ~400₽/мес = **1200–2000₽/мес**. Один наш сервер на всю семью = **250₽/мес**, и Smart TV/приставка тоже наконец работают.
