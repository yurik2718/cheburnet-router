...
2. Файл `.conf` от AmneziaWG → [Где взять ↓](#шаг-3--получить-amneziawg-конфиг)
...
Мастер спросит три вещи: адрес роутера, путь к `.conf` и название/пароль Wi-Fi — и сделает всё сам (~12 минут).
...
Нужен `.conf`-файл от VPN-сервера с поддержкой AmneziaWG:

**Вариант А — Amnezia Premium** (платная подписка, самый простой):
> В приложении: Настройки → Сервер → Поделиться → Экспорт конфигурации → скачайте `.conf`

**Вариант Б — Свой сервер** (бесплатно, нужен VPS):
> [amnezia.org](https://amnezia.org) — приложение сделает всё само за ~5 минут
...
2. Откройте [ifconfig.co](https://ifconfig.co) — посмотрите IP
3. Откройте yandex.ru — должен показывать российский IP
4. Откройте youtube.com — должен показывать IP вашего VPN-сервера
