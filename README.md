...
Список сайтов, которые идут напрямую, — **ваш**: мастер предлагает дефолт, свои домены вы добавляете
при настройке (изменить позже — «Настроить заново» в панели, две минуты). Домен (`example.com`)
покрывает и все его поддомены.
