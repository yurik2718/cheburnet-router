...
06-vpn-mode.sh           vpn-mode CLI, hotplug кнопки, init.d      ~1 мин
...
3. **Кнопка переключения режимов.** На Cudy TR3000 и Beryl AX кнопка работает через hotplug OpenWrt. На других роутерах нажатие кнопки может не срабатывать — переключайте через CLI: `vpn-mode home` / `vpn-mode travel`.

4. **Подсеть LAN.** Если у вас LAN не `192.168.1.0/24`, в `vpn-mode` и killswitch-rules нужно заменить.
