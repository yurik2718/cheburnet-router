...
- **Автоматически**: хендлером `/etc/hotplug.d/button/10-vpn-mode` при физическом переключении слайдера
- **Автоматически**: `/etc/init.d/vpn-mode` при загрузке (читает текущее положение GPIO)
- **Вручную**: `vpn-mode home | travel | toggle | status`

Подробно об управлении режимами → [docs/07-modes.md](07-modes.md). Об аппаратном переключателе Beryl AX — [docs/beryl-ax/slider-led.md](beryl-ax/slider-led.md).
