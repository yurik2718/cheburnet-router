...
### 5. Управление (физическая кнопка / CLI)

**Задачи:**
- Дать пользователю **физический** контроль без необходимости заходить в веб-интерфейс
- Обеспечить универсальное CLI (для автоматизации и пользователей без кнопки)

**Компоненты:**
- `gpio_button_hotplug` → `/etc/hotplug.d/button/10-vpn-mode` — **Cudy TR3000**: нажатие кнопки = `vpn-mode toggle`. **Beryl AX**: нажатие кнопки + чтение GPIO-слайдера при boot.
- CLI `vpn-mode` (home / travel / toggle / status / detect) — работает на любом роутере.
- `/etc/init.d/vpn-mode` — применяет последнее сохранённое состояние при каждой загрузке.
